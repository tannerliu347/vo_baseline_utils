...
from evaluate_ate_scale import align, plot_traj, plot_traj_est
...
class ATEEvaluator(object):
    ...
    def evaluate(self, gt_traj, est_traj, scale):
        ...
        est_our_frame = np.matrix(est_traj[:, 0:3].transpose())
        est_xyz = est_our_frame
        # est_xyz = np.zeros_like(est_our_frame)
        # est_xyz[1] = est_our_frame[2]
        # est_xyz[0] = est_our_frame[0]
        # est_xyz[2] = est_our_frame[1]
        ...
        est_xyz_aligned = np.array(est_traj_aligned)[:, 0:3].transpose()
        fig = plt.figure()
        ax = plt.gca()
        ts = np.arange(gt_xyz.shape[1])
        plot_traj(ax, ts, gt_xyz, "gt")
        plot_traj(ax, ts, est_xyz_aligned, "est")
        plt.show()
